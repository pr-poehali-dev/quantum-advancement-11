          ...
          {user ? (
            <>
              {(user.role === 'admin' || user.role === 'moderator') && (
                <ShimmerLink to="/admin" shimmerColor="#d4b96e" shimmerDuration="2.5s" borderRadius="8px"
                  className="px-4 py-2 text-xs font-medium text-gold-300 border-gold-500/40">
                  Админ
                </ShimmerLink>
              )}
            ...
            <>
              ...
              <ShimmerLink to="/register" shimmerColor="#d4b96e" shimmerDuration="2s" borderRadius="8px"
                className="px-4 py-2 text-xs font-medium text-gold-300 border-gold-500/40">
                Регистрация
              </ShimmerLink>
            </>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8">
                  ...
                  >
                    ...
                    {brandFilter && (
                      <span
                        ...
                        className="ml-1 text-gold-400/60 hover:text-gold-300 cursor-pointer"
                      >
                        <Icon name="X" size={12} />
                      </span>
                    )}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            {brandFilter && (
              <button onClick={() => setBrandFilter('')} className="mt-3 text-gold-400 text-sm hover:underline">
                Показать все бренды
              </button>
            )}
          </div>
...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link to={`/catalog/${product.id}`} className="group block">
      <div className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden hover:border-gold-500/40 hover:bg-white/8 hover:-translate-y-1 hover:shadow-xl hover:shadow-gold-500/10 transition-all duration-300">
        {/* Image */}
        <div className="aspect-square bg-gradient-to-br from-gold-500/10 to-purple-500/10 flex items-center justify-center relative overflow-hidden">
          {product.image_url ? (
          ...
          {isAlmostFull && !isBottle && (
            <div className="absolute top-3 right-3 bg-gold-500 text-white text-xs font-semibold px-2 py-1 rounded-full">
              Скоро выкуп
            </div>
          )}
          {isBottle && (
        ...
        <div className="p-4">
          ...
          <h3 className="text-white font-semibold text-sm leading-tight mb-2 group-hover:text-gold-300 transition-colors line-clamp-2">
            {product.name}
          </h3>
          ...
          {isBottle ? (
            <div className="flex items-center justify-between">
              <div>
                <span className="text-gold-400 font-bold text-lg">{Math.round(product.price_per_ml * product.bottle_ml)} ₽</span>
              </div>
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-gold-400 font-bold text-lg">{product.price_per_ml} ₽</span>
