  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4 items-end">
        ...
        <Button onClick={load} disabled={loading} className="bg-gold-500 hover:bg-gold-600 text-white h-9 px-5 text-sm">
          ...
          <thead>
            <tr className="border-b border-white/10 bg-white/3">
              <th className="px-3 py-3 w-10">
                <input type="checkbox" className="accent-gold-500 w-4 h-4 cursor-pointer"
            ...
            {!loading && users.map(u => (
              <tr key={u.id} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${u.is_blocked ? 'opacity-50' : ''}`}>
                ...
                <td className="px-3 py-3">
                  ...
                  {u.customer_code && (
                    <div className="text-gold-400/70 text-xs font-mono mt-0.5">{u.customer_code}</div>
                  )}
                  {u.admin_note && <div className="text-yellow-400/60 text-xs mt-0.5 italic">📝 {u.admin_note}</div>}
                </td>
                <td className="px-3 py-3 text-center">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${u.role === 'admin' ? 'bg-gold-500/20 text-gold-300' : u.role === 'moderator' ? 'bg-blue-500/20 text-blue-300' : 'bg-white/10 text-white/50'}`}>
                ...
                <td className="px-3 py-3">
                  <div className="flex items-center justify-center gap-1">
                    ...
                    <button onClick={() => { setEditUser(u); setEditForm({ ...u, newTag: '' }) }} title="Редактировать" className="p-1.5 rounded-lg text-white/30 hover:text-gold-400 hover:bg-gold-500/10 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setEditUser(null)}>
          <div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 w-full max-w-md space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 pt-1">
              <Button onClick={saveUser} disabled={saving} className="flex-1 bg-gold-500 hover:bg-gold-600 text-white">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setChatUser(null)}>
          <div className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-lg flex flex-col h-[560px]" onClick={e => e.stopPropagation()}>
            <div className="px-4 py-3 border-b border-white/10 flex items-center gap-2">
              <Icon name="MessageCircle" size={15} className="text-gold-400" />
            ...
            <div className="px-4 py-3 border-t border-white/10 flex gap-2">
              <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                ...
                rows={1}
                className="flex-1 bg-white/5 border border-white/15 text-white placeholder:text-white/25 rounded-xl px-3 py-2 text-sm resize-none outline-none focus:border-gold-500/50" />
              <Button onClick={sendReply} disabled={sending || !replyText.trim()} className="bg-gold-500 hover:bg-gold-600 text-white h-9 w-9 p-0 rounded-xl shrink-0">
