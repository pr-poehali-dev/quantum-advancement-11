    ...
    return (
      <div className="flex items-center gap-1">
        <input
          ...
          placeholder="№ выкупа"
          className="bg-zinc-800 border border-gold-500/50 text-white text-xs rounded-md px-2 h-7 w-20"
        />
        <button onClick={confirmDelivery} className="text-gold-400 hover:text-gold-300 text-xs px-1">✓</button>
    ...
    <>
      ...
      {ordersSubTab === 'archive' && <>
        <div className="flex flex-wrap gap-3 mb-4 items-end">
          ...
          <Button onClick={onLoadArchive} disabled={archiveLoading} className="bg-gold-500 hover:bg-gold-600 text-white h-9 text-sm px-5">
        ...
        <div className={`flex flex-wrap items-center gap-3 mb-4 rounded-xl px-4 py-3 border transition-all ${archiveSelected.size > 0 ? 'bg-zinc-700/30 border-zinc-500/40' : 'bg-white/3 border-white/10'}`}>
          <button onClick={() => { if (archiveSelected.size === archivedOrders.length) setArchiveSelected(new Set()); else setArchiveSelected(new Set(archivedOrders.map(o => o.id))) }}
            ...
            <input type="checkbox" readOnly checked={archivedOrders.length > 0 && archiveSelected.size === archivedOrders.length} className="accent-gold-500 w-4 h-4 pointer-events-none" />
              ...
              {!archiveLoading && archivedOrders.map(o => {
                ...
                return (
                  <tr key={o.id} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${archiveSelected.has(o.id) ? 'bg-white/5' : ''}`}>
                    <td className="px-3 py-3"><input type="checkbox" checked={archiveSelected.has(o.id)} onChange={() => setArchiveSelected(prev => { const n = new Set(prev); if (n.has(o.id)) { n.delete(o.id) } else { n.add(o.id) } return n })} className="accent-gold-500 w-4 h-4 cursor-pointer" /></td>
      ...
      <div className="flex flex-wrap gap-3 mb-4 items-end">
        ...
        <Button onClick={onLoad} disabled={loading} className="bg-gold-500 hover:bg-gold-600 text-white h-9 text-sm px-5">
      ...
      <div className={`flex flex-wrap items-center gap-3 mb-4 rounded-xl px-4 py-3 border transition-all ${selected.size > 0 ? 'bg-gold-500/10 border-gold-500/30' : 'bg-white/3 border-white/10'}`}>
        <button
          onClick={toggleAll}
          className="flex items-center gap-2 text-white/50 hover:text-white text-sm transition-colors"
        >
          <input
            ...
            className="accent-gold-500 w-4 h-4 pointer-events-none"
          />
          {selected.size > 0
            ? <span className="text-gold-300 font-medium">Выбрано: {selected.size} из {orders.length}</span>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {bulkStatus === 'delivery' && (
            <input
              ...
              placeholder="№ выкупа"
              className="bg-white/10 border border-gold-500/50 text-white text-sm rounded-md px-3 h-8 w-28 placeholder:text-white/30"
            />
          )}
          <Button
            onClick={onBulkStatus}
            disabled={applying || !bulkStatus || selected.size === 0}
            className="bg-gold-500 hover:bg-gold-600 text-white h-8 text-xs px-4 disabled:opacity-40"
          >
          ...
          <tbody>
            {loading ? (
              ...
              sorted.map(order => (
                <tr
                  key={order.id}
                  className={`border-b border-white/5 hover:bg-white/3 transition-colors ${selected.has(order.id) ? 'bg-gold-500/5' : ''}`}
                >
                  ...
                  <td className="px-3 py-3">
                    <Link to={`/catalog/${order.product_id}`} className="hover:text-gold-300 transition-colors">
                  ...
                  <td className="px-3 py-3 text-right">
                    <span className="text-gold-400 font-semibold">{order.total_price} ₽</span>
                  ...
                  <td className="px-3 py-3 text-center">
                    {order.pickup_batch
                      ? <span className="text-gold-400 font-mono font-semibold text-sm">№{order.pickup_batch}</span>
      ...
      {orders.length > 0 && (
        <div className="flex flex-wrap gap-6 mt-4 px-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gold-400 font-bold text-lg">{totalSum.toFixed(2)} ₽</span>
          </div>
        </div>
      )}
      </>}
