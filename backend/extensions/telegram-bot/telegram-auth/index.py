...
def cleanup_expired_tokens(cursor) -> None:
    """Mark expired auth tokens (no DELETE allowed by platform)."""
    pass
...
def find_user_by_telegram_id(cursor, telegram_id: str) -> Optional[dict]:
    ...
    schema = get_schema()
    cursor.execute(f"""
        SELECT id, email, nickname, avatar_url, telegram_id
        FROM {schema}users
        WHERE telegram_id = %s
    """, (telegram_id,))
...
def create_or_update_user(
    ...
    nickname = username or display_name
    ...
    if existing:
        ...
        cursor.execute(f"""
            ...
            WHERE telegram_id = %s
            RETURNING id, email, nickname, avatar_url, telegram_id
        """, (photo_url, telegram_id))
    else:
        # Create new user — email/phone/nickname required NOT NULL, password_hash NOT NULL
        cursor.execute(f"""
            INSERT INTO {schema}users (telegram_id, nickname, email, phone, password_hash, avatar_url)
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id, email, nickname, avatar_url, telegram_id
        """, (telegram_id, nickname, '', '', '', photo_url))
...
def cleanup_expired_refresh_tokens(cursor) -> None:
    """Stub — platform does not allow DELETE."""
    pass
